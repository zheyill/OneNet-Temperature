...
{
    /* 开启时钟 */
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_TIM1 | RCC_APB2Periph_GPIOA, ENABLE); // 开启 TIM1 和 GPIOA 的时钟

    /* GPIO 初始化 */
    GPIO_InitTypeDef GPIO_InitStructure;
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;
    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_11; // GPIO_Pin_11;  S
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_Init(GPIOA, &GPIO_InitStructure); // 将 PA11 引脚初始化为复用推挽输出

    /* 时基单元初始化 */
    TIM_TimeBaseInitTypeDef TIM_TimeBaseInitStructure;
    TIM_TimeBaseInitStructure.TIM_ClockDivision = TIM_CKD_DIV1; // 时钟分频，选择不分频
    TIM_TimeBaseInitStructure.TIM_CounterMode = TIM_CounterMode_Up; // 计数器模式，选择向上计数
    TIM_TimeBaseInitStructure.TIM_Period = period - 1; // 计数周期，即 ARR 的值
    TIM_TimeBaseInitStructure.TIM_Prescaler = prescaler - 1; // 预分频器，即 PSC 的值
    TIM_TimeBaseInitStructure.TIM_RepetitionCounter = 0; // 重复计数器
    TIM_TimeBaseInit(TIM1, &TIM_TimeBaseInitStructure); // 配置 TIM1 的时基单元
    ...
    TIM_OC4Init(TIM1, &TIM_OCInitStructure); // 配置 TIM1 的输出比较通道 4

    /* TIM 使能 */
    TIM_Cmd(TIM1, ENABLE); // 使能 TIM1，定时器开始运行
	TIM_CtrlPWMOutputs(TIM1, ENABLE);//开启PWM
}
